<?php
	...
	*/
	...
	class Payment{
		...
		protected static $tbl_name = "payments";
		function db_fields(){
			global $mydb;
		...
		*/
		...
		function number_installment_project($project_id=0){
			global $mydb;
			$mydb->setQuery("SELECT * 
			FROM payments
			WHERE payment_project_id= ".$project_id);
			$cur = $mydb->executeQuery();
			$row_count = $mydb->num_rows($cur);//get the number of count
			return $row_count;
		}
		
		function listOfProjectPayment($project_id=0){
			global $mydb;
			$mydb->setQuery("SELECT * 
			FROM payments
			WHERE payment_project_id= ".$project_id);
