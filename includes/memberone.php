	...
	*/
	...
	class User{
		...
		static function AuthenticateOfficer($email="", $h_upass=""){
			global $mydb;
			$mydb->setQuery("SELECT systemusers.user_account_id,user_name,account_password,account_type, bureau_id, school_id FROM `systemusers` JOIN officers ON systemusers.user_account_id = officer_id WHERE `user_name`='" . $email . "' and `account_password`='" . $h_upass ."' LIMIT 1");
			//$mydb->setQuery("SELECT staff_id, dname, email, level, password FROM staffs JOIN users ON staff_id = user_id WHERE `staff_id`='" . $email . "' and `password`='" . $h_upass ."' LIMIT 1");
			$cur = $mydb->executeQuery();
			$row_count = $mydb->num_rows($cur);//get the number of count
			if ($row_count == 1){
				...
				$_SESSION['ACCOUNT_TYPE']  = $found_user->account_type;
				$_SESSION['BUREAU_ID'] = $found_user->bureau_id;
				$_SESSION['SCHOOL_ID']  = $found_user->school_id;
				return true;
				}else{
				return false;
			}	
		...
		static function AuthenticateConsultant($email="", $h_upass=""){
			global $mydb;
			$mydb->setQuery("SELECT systemusers.user_account_id,user_name,account_password,account_type FROM `systemusers` WHERE `account_type`='consultant' and `user_name`='" . $email . "' and `account_password`='" . $h_upass ."' LIMIT 1");
			$cur = $mydb->executeQuery();
			$row_count = $mydb->num_rows($cur);//get the number of count
			if ($row_count == 1){
				$found_user = $mydb->loadSingleResult();
